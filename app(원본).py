...
def api_get():
    ...
    if sortMode == 'likes':
        movies = list(db.movies.find({'trashed': False}, {}).sort([('likes', -1), ('viewers', -1)]))
    elif sortMode == "viewers":
        movies = list(db.movies.find({'trashed': False}, {}).sort([('viewers', -1), ('likes', -1)]))
    elif sortMode == "date":
        movies = list(db.movies.find({'trashed': False}, {}).sort([('open_year', -1), ('open_month', -1), ('open_day', -1), ('likes', -1)]))
    else:
...
def api_get2():
    ...
    if sortMode == 'likes':
        movies = list(db.movies.find({'trashed': True}, {}).sort([('likes', -1), ('viewers', -1)]))
    elif sortMode == "viewers":
        movies = list(db.movies.find({'trashed': True}, {}).sort([('viewers', -1), ('likes', -1)]))
    elif sortMode == "date":
        movies = list(db.movies.find({'trashed': True}, {}).sort([('open_year', -1), ('open_month', -1), ('open_day', -1), ('likes', -1)]))
    else:
        return jsonify({'result': 'failure'})
...
def api_post():
    ...
    # 참고: '$set' 활용하기!
    result = db.movies.update_one({'title':title}, {'$set': {'likes':new_likes}})
